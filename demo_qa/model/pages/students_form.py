...
from demo_qa.model.controls import drop_down, radio_button, check_box, date_add
from demo_qa.utils import path_file
...
def select_birthday(*, month, year, day):
    ...
    date_add.select_date(month, year, day)
...
def select_hobbies(*texts):
    check_box.select(browser.all('[for^=hobbies-checkbox]'), *texts)


def upload_picture(path_to_picture):
    path_file.create_path('#uploadPicture', path_to_picture)
